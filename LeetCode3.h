...
namespace Test
{
    namespace LeetCode
    {
        ...
        int coinChange(const vector<int> &coins, int amount)
        {
            ...
            function<int(size_t, int)> count = [&](size_t i, int a) -> int
            {
                ...
                if (m.find(p) == m.end())
                {
                    ...
                    {
                        int c = count(i + 1, a); // coins[i] excluded
                        if (a >= coins[i])       // coins[i] included
                        {
                            int d = coins[i];
                            int c1 = 1;
                            while (d <= a)
        ...
        int coinChange2(const vector<int> &coins, int amount)
        {
            ...
            function<int(int)> count = [&](int a) -> int
            {
                ...
                if (a == 0)
                    return 0;
                if (change[a - 1] != 0)
                    return change[a - 1]; // amount a is at index a - 1
                int r = -1;
                for (int c : coins)
                {
                    int s = count(a - c);
        ...
        namespace CoinChange
        {
            ...
            static void ComputeMinCounts(unsigned int amount,
                                         vector<unsigned int> &denominations,
            ...
            static void ComputeSolution(unsigned int amount,
                                        vector<unsigned int> &denominations,
            ...
            static unsigned int GreedySolution(unsigned int amount,
                                               vector<unsigned int> &denominations,
                                               map<unsigned int, unsigned int> &changes)
            {
                function<bool(unsigned int, unsigned int)> greater =
                    [&](unsigned int first, unsigned int second) -> bool
                {
                    return first > second;
                };
                ...
                // Return what ever cannot be changed, e.g. the minimum denomination is
                // not 1.
                return amount;
            }
            ...
            static void
            ComputeAllSolutions(unsigned int amount, vector<unsigned int> &denominations,
                                vector<map<unsigned int, unsigned int>> &changes)
            {
                ...
                function<void(int, unsigned int, map<unsigned int, unsigned int> &)>
                    compute = [&](int sum, unsigned int minD,
                                  map<unsigned int, unsigned int> &s)
                {
                    for_each(denominations.begin(), denominations.end(),
                             [&](unsigned int d)
                             {
                                 if (d >= minD && sum >= (int)d)
                                 {
                                     // Since denominations is already sorted, we can
                                     // use minD as a gate to avoid to recompute
                                     // solutions with smaller denoms.
            ...
            static void
            ComputeSubSetSolutions(unsigned int amount, vector<unsigned int> &denominations,
                                   vector<map<unsigned int, unsigned int>> &changes)
            {
                // [TODO] no need to sort if use minIndex instead of minD in compute()
                sort(denominations.begin(), denominations.end());
        ...
        // 1 <= n <= 10^5
        // -100 <= nums[i] <= 100
        // 0    a[0]   a[1]     a[2]     ... a[k-2] a[k-1] a[k]   a[k+1] ... a[n-k-1] a[n-k]   a[n-k+1]  ... a[n-3]   a[n-2]   a[n-1]
        // 1    a[n-1] a[0]     a[1]     ... a[k-3] a[k-2] a[k-1] a[k]   ... a[n-k-2] a[n-k-1] a[n-k]    ... a[n-4]   a[n-3]   a[n-2]
        // ...
        // k    a[n-k] a[n-k+1] a[n-k+2] ... a[n-2] a[n-1] a[0]   a[1]   ... a[n-2k-1] a[n-2k] a[n-2k+1] ... a[n-k-3] a[n-k-2] a[n-k-1]
        // k+1  a[n-k-1] a[n-k] a[n-k+1] ... a[n-3] a[n-2] a[n-1] a[0]   ... a[n-2k-2] a[n-2k-1] a[n-2k] ... a[n-k-4] a[n-k-3] a[n-k-2]
        // F(k) =   0*a[n-k]   + 1*a[n-k+1] + 2*a[n-k+2] + ... + (k-1)*a[n-1] + k*a[0]     + ... + (n-2)*a[n-k-2] + (n-1)*a[n-k-1]
        // F(k+1) = 0*a[n-k-1] + 1*a[n-k]   + 2*a[n-k+1] + ... + k*a[n-1]     + (k+1)*a[0] + ... + (n-2)*a[n-k-3] + (n-1)*a[n-k-2]
        //        = 0*a[n-k-1] + a[n-k] + a[n-k+1] + ... + a[n-1] + a[0] + ... + a[n-k-3] + a[n-k-2] + F(k) - (n-1)*a[n-k-1]
        //        = a[n-k-1]   + a[n-k] + a[n-k+1] + ... + a[n-1] + a[0] + ... + a[n-k-3] + a[n-k-2] + F(k) - n*a[n-k-1]
        //        = Sum(a[0..(n-1)]) + F(k) + n * a[n-(k+1)]
