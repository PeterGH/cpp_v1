...
namespace Test
{
                          ...
                          int length2)
    {
        ...
        while (low <= high)
        {
            ...
            int m = (low + high) >> 1;
            int n = k - m - 2;
            // because 0 <= m <= min(k-1, s-1)
            // so k - min(k-1, s-1) - 2 = k - min(k, s) - 1 <= n <= k - 2
            // so        -1 <= n <= k - 2  if k <= s
            //    k - s - 1 <= n <= k - 2  if k > s
            if (n >= longLength)
            {
                           ...
                           int length2)
    {
        ...
        while (i < length1 || j < length2)
        {
            if (i >= length1)
            {
                return input2[j - 1 + k - c];
            }
            else if (j >= length2)
            {
                return input1[i - 1 + k - c];
            }
            else
            {
    ...
    template <class T>
    static T FindKthOrder3(int k, const T *input1, int length1, const T *input2,
                           int length2)
    {
        if (input1 == nullptr)
            throw invalid_argument("input1 is a nullptr");
                                           ...
                                           const T *input2, int length2)
    {
        ...
        if (length2 <= 0)
            throw invalid_argument(
                String::Format("length2 %d is invalid", length2));
        function<int(T, const T *, int)> search = [&](T v, const T *a, int l) -> int
        {
            int b = 0;
            int e = l - 1;
            while (b <= e)
    ...
    static size_t FindShiftPoint(const vector<int> &input)
    {
        ...
        if (*input.begin() < *(input.end() - 1))
            return 0;
        // Now the shift > 0
        // Following logic works only if shift > 0
        int b = 0;
        int e = input.size() - 1;
        // invariant is input[b] > input[e], which is maintained
        // in the loop
        while (b <= e)
        {
            int m = b + ((e - b) >> 1);
    ...
    public:
        KMP(const char *pattern)
        {
            ...
            // vs pattern[k+1]. The goal is for each i, find k such that
            // pattern[0..k] is suffix of pattern[0..i].
            int k = -1;
            this->prefix[0] = k;
