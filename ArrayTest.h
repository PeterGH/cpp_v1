...
void ArrayTest::Init(void) {
    ...
    Add("RowsColumns", [&]() {
        ...
        }
        {
            ...
            TransposeRowsToColumns<int>(B.get(), length, columns);
            Logger().Print(B.get(), length, columns);
            // Transpose element (m,n) to ((n % M), (k(m-1) + n/M)) means the
            // index i = Nm + n is mapped to j = (n % M)N + k(m-1) + n/M
            int k = columns / rows;
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < columns; j++) {
                    int s = i * columns + j;
        ...
        }
        {
            ...
            TransposeRowsToColumns<int>(B.get(), length, columns);
            Logger().Print(B.get(), length, columns);
            // Transpose element (m,n) to ((m%k)N + n, m/k)) means the index i =
            // Nm + n is mapped to j = (m%k)NN + nN + m/k
            int k = rows / columns;
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < columns; j++) {
                    int s = i * columns + j;
