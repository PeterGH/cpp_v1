...
void ArgumentTest::Init(void) {
    Add("wchar_t", [&]() {
        const wchar_t *wargv[6] = {L"-key1", L"value1", L"-key2",
                                   L"-key3", L"value3", L"value4"};
    ...
    Add("Argument_char", [&]() {
        int argc = 9;
        const char *argv[] = {"Test.exe", "I1",  "I2", "-K1", "V1",
                              "-K2",      "-K3", "V3", "I3"};
        Argument<char> arg(argc, argv);
        cout << arg.ToString() << endl;
        ASSERT2(
            arg.NamedArgCount() == 3,
            String::Format("arg.NamedArgCount() = %d", arg.NamedArgCount()));
        ASSERT2(arg.IndexedArgCount() == 4,
                String::Format("arg.IndexedArgCount() = %d",
                               arg.IndexedArgCount()));
        ASSERT2(arg.Count() == 7,
                String::Format("arg.Count() == %d", arg.Count()));
        ASSERT1(arg.Has(3));
    ...
    Add("Keys_char", [&]() {
        int argc = 9;
        const char *argv[9] = {"Test.exe", "I1",  "I2", "-K1", "V1",
                               "-K2",      "-K3", "V3", "I3"};
        Argument<char> arg(argc, argv);
        cout << arg.ToString() << endl;
        set<string> keys = arg.Keys();
        ASSERT2(keys.size() == 3,
                String::Format("keys.size() = %d", keys.size()));
    ...
    Add("Argument_wchar_t", [&]() {
        int argc = 9;
        const wchar_t *argv[9] = {L"Test.exe", L"I1",  L"I2", L"-K1", L"V1",
                                  L"-K2",      L"-K3", L"V3", L"I3"};
