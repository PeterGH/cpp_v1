...
void LeetCodeTest::Init3(void)
{
    ...
    Add("393. UTF-8 Validation", [&]()
        {
            auto check = [&](const vector<int> &v, bool e)
            {
                Logger() << v;
                bool a = LeetCode::validUtf8(v);
                Logger() << "is valid utf8: " << a << endl;
                ASSERT1(a == e);
            };
            check({197, 130, 1}, true);
    ...
    Add("397. Integer Replacement", [&]()
        {
            auto check = [&](int n, int e)
            {
                ...
                int c4 = LeetCode::integerReplacement4(n);
                Logger() << n << "->1: " << c << ", " << c2 << ", " << c3 << ", " << c4 << ", " << e << endl;
                if (e >= 0)
                {
                    ASSERT1(c == e);
                    ASSERT1(c2 == e);
